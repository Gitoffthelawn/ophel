...
`obstacleSelectors` 用于声明宿主页面自己打开的侧栏、Canvas、文档、深度研究或思考面板；这些原生面板会和 Ophel 面板一起参与安全区计算。`PanelAvoidanceInsetConfig` 可选声明自己的 `scopeSelector`、`applySide` 和 `insetMode`。默认复用主 `scopeSelector`、同时应用左右 inset，并使用包含内容居中余量的 `centered` 模式；当站点存在聊天区之外的固定右栏或兄弟面板时，可以为该面板单独声明更大的计算 scope，使用 `edge` 模式只取避让面板所需的边缘余量，避免主聊天区未重叠时整段避让样式被清掉，也避免右栏被内容居中余量额外推开。
...
5. 读取站点声明的 `scopeSelector` 作为内容区域；未声明时才退回整个 viewport。查找会穿透页面 Shadow DOM，以支持 Gemini Enterprise 这类站点。
6. 读取 Ophel 面板和 `obstacleSelectors` 命中的宿主原生面板 `getBoundingClientRect()`，只计算它们与 scope 相交的左右占用区。
...
11. 生成 `#gh-panel-avoidance-styles`，注入主文档 `document.head`；同时生成 Shadow DOM 版本并通过 `#gh-panel-avoidance-shadow` 注入允许的页面 Shadow Root。
...
因此 DeepSeek 历史对话使用主消息滚动区作为 scope，并同时覆盖消息列表和输入区的左右 padding：
...
新对话页没有 `.ds-message`，历史对话 scope 不会命中。页面中的标题、模式切换和输入框同属一个 `max-width: 840px` 的中心内容块，外层全屏容器可用 `#root > div:has(textarea.ds-scroll-area):not(:has(.ds-message))` 命中；避让时只给这个全屏容器加 `edge` inset，让中心内容块在剩余安全区内重新居中，避免直接压缩输入框本体。

Canvas 打开后，DeepSeek 会形成左侧聊天、右侧 iframe 预览的双栏布局。右侧预览栏不是聊天虚拟列表的子节点，左侧 scope 不会与右侧 Ophel 面板相交；需要以包含 `.ds-virtual-list` 和右侧可见 iframe 的根容器作为 scope，单独给右侧预览栏内的 `.ds-scroll-area` 加右侧 `edge` inset，并强制使用 border-box，让 iframe 预览在安全区内收缩，同时保持左侧聊天列宽度不变。
...
- 新对话页没有消息虚拟列表，问候语和输入框同属一个全高居中容器；输入框边框、阴影和按钮在 `input-content-container` 内部，不能直接给这层加 padding，否则只会移动内部控件而边框仍然遮挡面板。
- Canvas 页面把右侧预览放在独立 `aside` 里，实际 iframe 位于 `.code-canvas` 下；不能给 `aside` 或绝对定位外层加 `margin-right`，否则整个右栏会左移并压到聊天列。
...
4. 对新对话页的全高居中容器使用 `edge` inset，只预留 Ophel 面板实际占用的边缘空间，让问候语和输入框外框作为整体避让。
5. 对 Canvas 右栏使用独立 `aside:has(.code-canvas)` scope，并只给 `.code-canvas` 加右侧 `edge` padding，让 iframe 和顶部工具区缩进到 Ophel 面板左侧。
...
| IMA | 页面内容容器 | 分别重算回复容器和编辑器容器 `width` | 对主区域、输入容器和新对话页内容加左右安全区 | `960px` |
...
| Gemini Enterprise | `mat-sidenav-content` / `.main.chat-mode` | 复用 Enterprise 宽度选择器并注入页面 Shadow DOM | 主内容区和输入区加左右安全区 | `760px` |
...
| Grok | `main[data-mcp-app-fullscreen-container]` / Canvas 用 `#grok-app-root` | 通过 `--content-max-width` 变量统一控制消息区和输入区 | 聊天滚动区、空态、底部输入区和 Canvas 预览区加安全区 | `768px` |
...
- 历史对话页只对 `.chat-detail-main` 加 `padding-left/right`，避免外层 `#chat-container.layout-content` 和内层主区域重复 inset 导致避让状态闪烁。
- 新对话页没有 `.chat-detail-content` 时，才对 `#chat-container.layout-content` 使用 `edge` inset，让首页输入区进入安全区，同时不影响历史对话页。
...
- 对聊天内容区和输入区以左侧聊天 Pane 为 scope 分别加左右安全区，保持两者位置一致，同时避免右侧 Canvas 打开时把左侧聊天区二次压窄。
- Canvas 画布打开后，使用 `SplitPane` 作为整体 scope，对右侧 `.Pane2` 单独应用右侧 edge inset，让画布从 Ophel 面板下方移开，并保持左侧聊天 Pane 宽度不变。
- Canvas 内的代码/文档预览由 Monaco Editor 渲染，容器变窄后不能靠普通 CSS 换行；扩展端通过元宝专属 MAIN world 脚本、油猴端通过 `unsafeWindow` 调用同一安装函数，对 Canvas 内 Monaco 实例设置 `wordWrap: "on"` 并在尺寸变化后调用 `layout()`。
...
- 对对话页主区域和输入容器都加左右安全区，避免输入框留在面板下方。
- 新对话页的 logo、输入框和精选示例位于独立滚动容器中，使用 `edge` inset 只扣除面板占用区，让它们在剩余安全区内重新居中，同时保留输入框与精选示例自身的响应式宽度；新对话页输入框不再叠加对话页输入容器 inset。
...
- Z.ai 新对话页的标题、输入框和推荐入口同属首页内容容器；该页面不再给 `.messageInputContainer` 单独叠加安全区 padding，而是对包含 `.placeholder-input` 的首页内容容器使用 `edge` inset，让整块内容在扣除 Ophel 面板后的安全区内重新居中。
- ChatGLM 避让时重算响应、消息和 Markdown 容器，输入底部容器继续由页面宽度控制处理。
- ChatGLM 新对话页的首页引导区 `.main-chat-guide-container` 是绝对定位，不能只依赖 `.conversation-inner` 的安全区 padding；单独对该引导区使用 `edge` inset，让标题、输入框和快捷入口一起进入安全区，同时保留输入框自身宽度。
- ChatGLM Canvas 打开后，右侧 `.code-preview-tabs` 与左侧聊天区同属外层 `section.el-container` 的兄弟节点；左侧聊天 scope 不会与右侧 Ophel 面板相交，需要以整个双栏容器为 scope 单独给 Canvas 预览区加右侧 edge inset，让 iframe 预览跟随安全区收缩。
...
- Canvas 画布打开后，继续以 `#qianwen-left-panel` 计算聊天区避让，同时对 `.splitCardContainer` 下的右侧 Canvas 面板使用独立右侧 edge inset；千问双栏由 flex-grow 分配，避让时让左右列按剩余空间重排，避免 Canvas 被 Ophel 面板覆盖。
...
- 避让时只重算消息宽度；历史对话页输入区只对外层 `.chat-layout-input-container` 使用安全区 padding，`.message-input-wrapper` 只保留页面宽度控制，避免输入框自身被安全区 padding 二次压窄；新对话页没有 `.chat-left-panel` 时，再单独对 `.message-input-wrapper` 使用 `edge` inset，恢复首页输入框避让。
...
- 使用 `bard-sidenav-content` 作为首选 `scopeSelector`，新对话页没有该节点时回退到 `main.chat-app`。
- 避让时同时重算 `.conversation-container` 和 `.input-area-container` 的 `max-width`，让消息列和输入框使用同一个安全区目标宽度。
- Deep Research、Canvas 和 Drive 文档查看器打开后，`chat-window.immersives-mode` 只在右侧按 edge inset 收敛可用宽度，并减掉 Gemini 沉浸面板自带的右侧内间距，让聊天列和沉浸面板作为同一个网格重算宽度，避免整体平移出可视边界，也避免左右两块分别避让后留下大块空白。
- 对 `infinite-scroller.chat-history` 和 `input-container` 以聊天列 `.chat-container` 为 scope 加左右安全区，让消息区和输入区位置同步，避免右侧沉浸面板打开时把左侧对话列二次压窄。
- 新对话页的零态输入框是绝对定位，不能只依赖 `input-container` 的安全区 padding；单独对 `.input-area-container.is-zero-state` 使用 edge inset 重设左右边界，让输入框在扣除 Ophel 面板后的安全区内居中。
- 输入框外层仍保留在安全区容器内，避免只改输入框宽度但位置仍落在面板下方。
- Deep Research、Canvas 和 Drive 文档查看器不作为聊天区 `obstacleSelectors`，避免 Gemini 自己的右侧面板把左侧对话列二次压窄。

### Gemini Enterprise

Gemini Enterprise 页面内容在页面自己的 Shadow DOM 中，普通 document 样式和 document 查询不能完整覆盖。

适配策略：

- `LayoutManager` 的 scope 与 obstacle 查找穿透页面 Shadow DOM。
- 避让样式除注入 `document.head` 外，还通过 `#gh-panel-avoidance-shadow` 注入允许的页面 Shadow Root。
- 复用 Enterprise 已有页面宽度选择器，避免再维护一套平行宽度规则。
- 使用 `mat-sidenav-content, .main.chat-mode` 作为 scope，输入区 `.input-area-container` 单独加入 inset。
...
- 对 `#thread` 内定义并消费 `--thread-content-margin` 的外层节点使用 `edge` 安全区，让消息 turn 和 composer 一起进入可见区域；新对话页问候标题不在这条外层链路里，单独对标题容器使用同样的 `edge` inset，使标题、输入框和提示入口保持同一安全区中心。
- 新对话页底部 starter prompt 横向滚动行也会消费 `px-(--thread-content-margin)`，但它只负责卡片列表的内部留白，不能再叠加面板安全区 padding，否则提示卡片会被压缩到很窄。
- 不对 `form[data-type="unified-composer"]` 本身加 inset；新版新对话页里 form 已在 `thread-content-margin` 容器内，若 form 同时被设置 `max-width` 和大额 padding，会把输入框内部压缩到几乎不可见。
- Canvas 页面使用 fixed 全屏代码块编辑器 dialog 承载代码栏、顶部工具栏和右侧预览 iframe，不属于普通消息列；对这个 dialog 单独应用右侧 `edge` inset，让隐藏代码和显示代码两种状态下的预览区、顶部工具按钮一起收进 Ophel 面板左侧，同时保留 dialog 原本左侧 padding。
- 库文件查看页使用 writing-block fullscreen shell：正文编辑器在 `fullscreen-shell-body` 内，底部“描述修改内容”输入框是同一 dialog 下的 fixed sibling。页面宽度控制需要同时覆盖正文编辑器和 fixed 输入框外层；智能避让时对 shell 应用 `edge` inset，并把 fixed 输入框外层从 `start-1/2 + translate` 改成安全区左右定位，再让内部 form 按页面宽度居中。
...
- 使用 `.chunk-editor-main` 作为主 `scopeSelector`，让右侧 Run settings 面板自然从聊天可用宽度中扣除；Run settings 不参与主内容的 `obstacleSelectors`，否则新对话 zero-state 会在设置栏左移前后反复重算。
- 避让时同时重算 `.chat-session-content`、`.chat-turn-container` 和底部 `footer ms-prompt-box` 的 `max-width`，保证消息区和输入框宽度一致。
- 对 `.chat-container .chat-view-container` 和 `footer` 加 `edge` 安全区，避免 centered 模式把新对话的模型卡片和输入框压窄。
- 右侧 `ms-right-side-panel` 使用 `ms-chunk-editor` 作为独立计算 scope，并用 `edge` 模式写入 CSS 变量后通过 `translate` 视觉左移，让 Run settings 自身从 Ophel 面板下方移开，同时不改变 `ms-chunk-editor` 的 flex 宽度，避免新对话页在两个布局间闪烁。
...
- 对聊天滚动区、新对话页 Logo 空态和底部 `max-w-breakout` 输入宿主加左右安全区；Logo 空态命中包含 `svg[variant="hero"]` 的 `max-w-breakout` 外层，输入宿主在避让时放开自身 `max-width`，由内部输入框继续消费 `max-w-breakout`，避免安全区 padding 把输入框压窄。
- Canvas 页面的右侧预览 pane 不在 `main[data-mcp-app-fullscreen-container]` 内，而是 `#grok-app-root` 下的 sibling；对包含 `iframe.w-full.flex-1` 的 `aside` 单独用 `#grok-app-root` 计算右侧 `edge` inset，让 iframe 和顶部预览工具栏一起收进 Ophel 面板左侧，同时不移动左侧聊天/代码区。
