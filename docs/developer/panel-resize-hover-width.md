...
hover-width 生效时，拖拽仍遵循直接操控当前可见边缘：从当前可见宽度开始计算拖拽，真实拖拽结束后将结果写入 `panel.width`，并临时抑制普通 hover-width，直到鼠标离开面板再恢复悬停加宽能力。不要在 pointerdown 时切回隐藏的基础宽度，否则手柄会跳离指针；结束后的可见宽度应立即代表新的基础宽度。
...
- “交互层”和“保留悬停宽度的交互层”是两个概念：设置弹窗、设置搜索等配置上下文仍属于 Ophel 交互层，但不保留悬停宽度；打开这些配置层时必须主动释放普通 hover-width 状态，不依赖浏览器是否触发 `mouseleave`。从面板内打开的菜单、小对话框、提示词预览等使用 hover-width retain 标记，关闭后再判断是否恢复基础宽度。
...
- 拖拽调整基础面板宽度属于用户显式操作，优先级高于普通面板 hover/focus/retain 触发的悬停加宽；设置弹窗“悬停宽度”行的显式预览不受该抑制影响。
...
- 打开设置弹窗或全局搜索这类配置层时，曾依赖覆盖层插入后的 `mouseleave` 释放 hover-width。后续必须通过显式释放信号清空普通 hover/focus/retain 状态，让配置层首帧稳定使用基础面板宽度。
...
- hover-width 生效时拖拽调整基础面板宽度，结束后曾会因为鼠标仍在面板内而立刻恢复到悬停宽度。真实拖拽现在从当前可见边缘开始，结束后把结果保存为新的基础宽度，并清除 hover-width retain、抑制普通 hover-width，直到鼠标离开面板。
- 全局搜索曾遗漏“悬停时加宽”和“悬停宽度”两个设置项。搜索索引必须包含这两个设置；在悬浮模式下从搜索结果定位它们时，应先展开“面板宽度”手风琴再滚动高亮目标行。
...
- hover 临时加宽状态下拖拽调整面板宽度，拖拽从当前可见边缘开始，pointerup 后立即显示并保存为新基础宽度；鼠标离开后再重新进入，才恢复悬停加宽。
- 开启悬停加宽后，普通悬浮面板 mouse enter 加宽、mouse leave 收回。
- 开启悬停加宽后，打开并取消新建文件夹、菜单、对话框等交互层，鼠标不在面板内时面板会自动恢复基础宽度。
- 开启悬停加宽后，打开设置弹窗或全局搜索时面板会主动释放普通 hover-width，不再因为覆盖层是否触发 `mouseleave` 而停留在悬停宽度；调整基础面板宽度能立即看到效果。
...
- 全局搜索能通过“悬停时加宽”“悬停宽度”以及相关中英文关键词找到这两个设置；悬浮模式下点击搜索结果能自动展开“面板宽度”并高亮对应子项。
