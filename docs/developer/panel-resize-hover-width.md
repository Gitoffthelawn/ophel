...
真实拖拽结束后，若鼠标仍停留在面板内，应临时抑制普通 hover-width，直到鼠标离开面板再恢复悬停加宽能力。这样用户能立即看到刚写入的基础面板宽度，不会在 pointerup 后被悬停宽度抢回视觉结果。
...
- hover-width 生效时拖拽调整基础面板宽度，结束后曾会因为鼠标仍在面板内而立刻恢复到悬停宽度。真实拖拽结束后现在会清除 hover-width retain 并抑制普通 hover-width，直到鼠标离开面板。
...
- hover 临时加宽状态下拖拽调整面板宽度，pointerup 后立即显示新基础宽度；鼠标离开后再重新进入，才恢复悬停加宽。
